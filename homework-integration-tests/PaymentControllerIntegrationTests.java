...
public class PaymentControllerIntegrationTests {
    ...
    private static AccountModel.Currency wrongCurrency;
    private static List<AccountModel> noUserIDAccount;
    ...
    public static void setup(){
       ...
       wrongCurrency = AccountModel.Currency.RON;
       ...
       noUserIDAccount = new ArrayList<>();
       noUserIDAccount.add( new AccountModel(id(), "", AccountModel.Currency.RON, 10000));
    }
    ...
    @Test
    public void sendPaymentWrongCurrency() throws Exception {
        when(repository.getUser(sender.getEmail())).thenReturn(Optional.of(sender));
        when(repository.getUserAccounts(sender.getId())).thenReturn(senderAccounts);
        when(repository.getUser(receiver.getEmail())).thenReturn(Optional.of(receiver));
        when(repository.getUserAccounts(receiver.getId())).thenReturn(receiverAccounts);
        ...
        String endpoint = "/api/payments?receiver=%s&currency=%s&amount=%f";
        mockMvc.perform(MockMvcRequestBuilders.post(
                String.format(endpoint, receiver.getEmail(), wrongCurrency, 10* senderAccounts.get(0).getAmount()))
                .header((Credentials.AUTHORIZATION), credentials(sender)))
                .andExpect(MockMvcResultMatchers.status().isInternalServerError())
                .andExpect(MockMvcResultMatchers.jsonPath("$").value(PaymentException.userHasNoAccountForCurrency().toString()));
    ...
    //DELETE/api/user badCredentials
    @Test
    public void nonAdminDeleteUser() throws Exception {
        when(repository.getUser(sender.getEmail())).thenReturn(Optional.of(sender));
        when(repository.getUserAccounts(sender.getId())).thenReturn(senderAccounts);
        when(repository.getUser(receiver.getEmail())).thenReturn(Optional.of(receiver));
        when(repository.getUserAccounts(receiver.getId())).thenReturn(receiverAccounts);
          String endpoint = "/api/users?email=%s&currency=%s";
        mockMvc.perform(MockMvcRequestBuilders.delete(
                String.format(endpoint, receiver.getEmail(),senderAccounts.get(0).getCurrency()))
                .header((Credentials.AUTHORIZATION), credentials(sender)))
                .andExpect(MockMvcResultMatchers.status().isForbidden())
    ...
    //PUT /api/accounts accountCouldNotBeSaved
    @Test
    public void updateAccountNoCurrency() throws Exception {
        when(repository.getUser(sender.getEmail())).thenReturn(Optional.of(sender));
        when(repository.getUserAccounts(sender.getId())).thenReturn(senderAccounts);
        String endpoint = "/api/accounts";
        mockMvc.perform(MockMvcRequestBuilders.put(
                ...
                .header((Credentials.AUTHORIZATION), credentials(adminEmail, adminPassword))
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(noUserIDAccount.get(0))))
                .andExpect(MockMvcResultMatchers.status().isInternalServerError())
                .andExpect(MockMvcResultMatchers.jsonPath("$").value(PaymentException.accountCouldNotBeSaved().toString()));
    }

    //DELETE /api/accounts accountCouldNotBeRemoved
    @Test
    public void deleteAccountNoEmail() throws Exception {
        when(repository.getUser(sender.getEmail())).thenReturn(Optional.of(sender));
        when(repository.getUserAccounts(sender.getId())).thenReturn(senderAccounts);
